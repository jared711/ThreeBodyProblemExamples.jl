...
# Plot the halo orbit
pxy = plot(halo,idxs=(1,2),legend=false,title="Halo Orbit",xaxis="x",yaxis="y"); # plot the halo orbit in the x-y plane
pyz = plot(halo,idxs=(2,3),legend=false,title="Halo Orbit",xaxis="y",yaxis="z"); # plot the halo orbit in the y-z plane
pxz = plot(halo,idxs=(1,3),legend=false,title="Halo Orbit",xaxis="x",yaxis="z"); # plot the halo orbit in the x-z plane
pall = plot(halo,idxs=(1,2,3),legend=false,title="Halo Orbit"); # plot the halo orbit in 3D
...
# Let's make a function that encapsulates the entire mapping process
function strob_map(rv₀, ξ, sys)
    # u is the initial invariant circle
    # T is the period of the orbit
    # ρ is the rotation number
    u = ξ[1:end-2]
    u = [u[i:i+5] for i in 1:6:length(u)] # Convert u to a vector of vectors
    ρ = ξ[end-1]
    T = ξ[end]

    # N is the number of points to use in the invariant circle
    N = length(u)

    # First we integrate the orbit
    tspan = (0.,T) # integrate from 0 to T₀
    prob_halo = ODEProblem(CR3BPdynamics!,rv₀,tspan,sys) # CR3BPstm! is our in-place dynamics function for state and STM
    
    function prob_func(prob, i, repeat)
        remake(prob, u0=rv₀+u[i]) # perturb rv₀ by the ith point of the invariant circle and use that as the initial condition
    end # NOTE: Do not get confused! ODEProblems have an field called "u0" (e.g. prob_halo.u0) and the solved problems have a field called "u" (e.g. halo.u). Don't confuse these with the u₀ and u variables that we are defining here.
    prob_qpo = EnsembleProblem(prob_halo, prob_func=prob_func) # ODE problem with an ensemble of trajectories
    qpo = solve(prob_qpo, TsitPap8(), trajectories=N, abstol=1e-12, reltol=1e-12) # solve the problem
...
ξ = [reduce(vcat,u₀); # reduce(vcat,u₀) turns u₀ into one big long vector instead of a vector of vectors 
                  ρ₀;
                  T₀]
J_fd = zeros(6N+1, 6N+2)
γ = strob_map(rv₀, ξ, sys)
for i = 1:length(ξ)
    δ = zeros(length(ξ))
    δ[i] = 1e-6
    J_fd[:,i] = (strob_map(rv₀, ξ+δ, sys)-γ)/δ[i]
    if i < length(ξ)-1
        J_fd[i,i] -= 1 
    end
end
# uTR_2, C_2 = strob_map(rv₀, [u₀[i]+1e-6.*ones(6) for i in 1:N], T₀, ρ₀, sys)
...
J = [J₁  J₂  J₃;
      J₄  0  0;
      1 zeros(1,6N) 0] # Add the constraint that the average velocity is zero
...
plot(qpo,idxs=(1,2,3),legend=false,title="Approximate QPO") # Plot the approximate quasi-periodic orbit

u⁽ⁱ⁺¹⁾ = 1


function plot_us(us;N=[])
    if isempty(N)
       N = length(us)
    end
    plot_iter = plot()
    for (i,c) in enumerate([:red,:blue,:green,:orange,:purple,:brown,:pink,:gray,:olive,:cyan])
       if i > N
           break
       end
       plot!(plot_iter,us[i],legend=true,label=string("u",i),linecolor=c); # Plot the invariant circle after integrating
    end
    display(plot_iter)
end
...
u₀,ρ₀ = invariant_circle(rv₀,T₀, 19, sys, α=1e-3)
u, T, ρ, C, us, uTRs, Cs = differential_corrector_QPO(sys, rv₀, u₀, T₀, ρ₀, max_iter=5,ϵ=1e-12);
plot_us(us)